...
**恢复路径中优先使用数据面操作，尽量减少控制面依赖。**

> AWS 白皮书的理想建议是"恢复路径不依赖控制面"，但实际客户环境中很难 100% 做到（未预置的资源、需要临时调整的配置等）。因此采用**分级策略**而非硬规则。

| 风险等级 | 操作类型 | 计划中的标注 | 示例 |
|----------|---------|-------------|------|
| 🟢 **推荐** | 数据面操作 | 无特殊标注 | RDS promote replica、Route 53 健康检查触发切换、CloudFront origin failover |
| 🟡 **注意** | 控制面操作（Region 内） | `⚠️ 控制面操作` | 修改 Target Group、更新 Security Group、调整 Auto Scaling |
| 🔴 **高风险** | 控制面操作（跨 Region 依赖） | `🔴 依赖远程控制面` | 创建 ELB（依赖 Route 53 CP @ us-east-1）、创建 S3 桶（依赖 us-east-1）、修改 Route 53 记录 |

**Step Builder 生成命令时标注操作类型**（数据面 / 控制面-区域内 / 控制面-跨 Region），帮助用户评估风险，但不强制阻断。高风险操作建议（非强制）移到 Phase 0 预置。

#### 5.4.2 静态稳定性检查

Phase 0 预检**建议**验证以下项目，未通过时生成告警（非阻断）：

1. DR 目标中关键资源是否已预置（ELB、RDS 副本、S3 桶、IAM 角色）— 未预置时标注 🔴
2. 切换步骤是否依赖跨 Region 控制面操作 — 存在时标注 🔴
3. Route 53 切换是否基于健康检查（数据面）而非记录修改（控制面）— 控制面修改标注 🟡
4. STS 是否配置为区域端点 — 全局端点标注 🟡
5. Global 服务依赖是否已识别 — 未识别标注 🟡

> **设计哲学**：告知风险，不阻断执行。客户可能有充分理由使用控制面操作，工具的职责是标注风险而非替客户决定。
