...
def _dependency_edge_labels() -> list:
    ...
    ## 两个来源，因为本文件有两份部署形态（2026-09-06）

    `rca/neptune/` 下能按相对路径读到 `profiles/graph_contract.yaml`；
    但 `infra/lambda/rca_window_flush/neptune/` 那份副本的三级父目录里没有
    `profiles/`，只能走兜底 —— 而兜底清单**漂移了**：它少了 `Invokes`
    （契约 7 类，兜底 6 类）。Lambda 里因此会漏掉 16 条 `Invokes` 边。

    改法是加一个来源：Lambda 的 `neptune-client-base` 层里有
    `graph_contract.EDGE_TYPES`（契约已烧成 Python 模块），优先用它。
    顺序 = 层模块 → YAML 文件 → 兜底，任一命中即返回。

    兜底清单与契约漂移由 tests/test_52 的方向门禁一并钉住。
    """
    try:
