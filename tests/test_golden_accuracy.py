...
默认 skip（成本考虑）。启用：
  cd rca && PYTHONPATH=.:.. RUN_GOLDEN=1 NLQUERY_ENGINE=direct  # ← 已失效，direct 已删除  pytest ../tests/test_golden_accuracy.py -v
...
⚠️ 2026-09-21：本文件头原先写的运行命令带 `NLQUERY_ENGINE=direct`，
...
七份各自维护的命令必然漂移，这次就漂了）：
    ...
    bash scripts/run_golden_suite.sh smart_query   # 只跑本套
