...
Usage:
  ...
  cd chaos/code && PYTHONPATH=. RUN_GOLDEN=1 POLICY_GUARD_ENGINE=direct  # ← 已失效，direct 已删除  pytest ../../tests/test_policy_guard_golden.py -v -s
  cd chaos/code && PYTHONPATH=. RUN_GOLDEN=1 POLICY_GUARD_ENGINE=strands pytest ../../tests/test_policy_guard_golden.py -v -s

⚠️ 2026-09-21：本文件头原先写的运行命令带 `POLICY_GUARD_ENGINE=direct`，
...
七份各自维护的命令必然漂移，这次就漂了）：
    ...
    bash scripts/run_golden_suite.sh policy_guard   # 只跑本套
