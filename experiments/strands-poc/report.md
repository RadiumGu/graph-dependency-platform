...
## 8. L2 Prompt Caching 效果（2026-04-18）
...
| # | Commit | 范围 |
|---|--------|------|
| 1 | `8f82523` | Direct 引擎 `system` 改 list + `cache_control=ephemeral`；token_usage 加 `cache_read` / `cache_write` |
| 2 | `6840e24` | Strands `BedrockModel` 加 `cache_prompt='default'` + `cache_tools='default'`（SDK 原生支持，无需 hack） |
| 3 | `9437913` | Golden + Shadow 增 `Avg Cache Hit Ratio` + `Cache read/write tokens` 列 |
| 4 | 本次    | report.md §8 + CUSTOMER-REFERENCE 9.1 |

### 8.2 基线对比（相同 20 golden cases，2026-04-18）

| 指标 | L1 Baseline | L2 Baseline | 变化 |
|------|-------------|-------------|------|
| Direct pass | 20/20 | 20/20 | 持平 ✅ |
| Direct p50 latency | 5458 ms | 5202 ms | -4.7% |
| Direct total tokens | ~123k | ~116k | -6% |
| Direct Avg Cache Hit Ratio | N/A | *99.6%* | 新增 |
| Strands pass | 20/20 | 20/20 | 持平 ✅ |
| Strands p50 latency | 9269 ms | 9823 ms | +6% |
| Strands total tokens | — | ~463k | （L1 未统计）|
| Strands Avg Cache Hit Ratio | N/A | *90.9%* | 新增 |

> 延迟变化主要受 Bedrock 运行时波动影响，非 caching 本身贡献；但 direct `verify_cache_direct.py` 的同问 3 次循环显示 *第 2/3 次延迟从 7.8s → 4.1s（-48%）*，说明 warm-cache 单次调用节省明显，受 golden set 20 个"独立前缀稍有差异"的问题稀释。

### 8.3 月度成本外推

基于 Sonnet 4.6 官方单价（$3 / $3.75 / $0.30 / $15 per 1M），用 Golden 总 token 按同量级外推到 15k queries/month（50 DAU × 10 q/day × 30d）：

| 引擎 | 无缓存估算 | 启用缓存后 | 节省 |
|------|-----------|-----------|------|
| Direct | ~$441/月 | ~$229/月 | *-48%* |
| Strands | ~$1312/月 | ~$505/月 | *-62%* |

两个引擎都远超验收门槛（direct ≥35%, strands ≥45%）。

### 8.4 陷阱 / 踩坑

1. *最低缓存 size*: Sonnet/Opus 最低缓存 ~1024 tokens (~3000 chars)。Profile 变简精后若 system prompt < 1024 token，Bedrock 会静默忽略缓存。已加 `assert len(system_prompt) > 3000` 防御，当前 PetSite system prompt ~12060 chars 远超门槛。
2. *Profile 改动 → cache 失效*: Wave 2 改 few-shot 后，下一次调用付 125% write 成本（warm-up）再回到稳态。正常行为，别加锁。
3. *Opus 独立缓存池*: Wave 5 关键词命中升 Opus 时，第一次 Opus 调用是 cache miss。如果 Opus 调用频率低（< 1 query/5min），可能每次都 miss。观察到命中率与 query 频率正相关，必要时给 Opus 加"预热脚本"。
4. *ReAct 多轮 cache 内循环*: Strands 1.36 `cache_prompt='default'` + `cache_tools='default'` 会把 3 个 tool schema 的 JSON（~800 tokens）+ system prompt 都缓存；*单次 query 内* cycle 2/3 就能命中 cycle 1 刚写入的缓存，所以 run 1 就有 cache_read（~12k/6k = 2x 的内循环命中）。
5. *latency 未必降*: ReAct 多 cycle 决定延迟下界；缓存主要省成本，不省时间。
6. *`cache_prompt` 已 deprecated*: SDK 建议用 `cache_config=CacheConfig(strategy="auto")`，但当前仍然工作。Phase 3 升级 strands SDK 时再切换。

### 8.5 Phase 3 启动前补作业（刷新后）

- [ ] 主环境 strands 依赖管理正式化（L1 稳定 4 周后）
- [x] *L2 Prompt Caching 接入*（2026-04-18, direct `8f82523` + strands `6840e24`）
- [x] Golden + Shadow 增命中率统计（2026-04-18, `9437913`）
- [x] Strands OTel hook（`f4d7665`）
- [x] `check_migration_deadlines.py` 接 CI（`fc0ac36`）
- [x] q014 prompt 调优（`ecd17cb`）

唯一剩下的挂起项是"等 4 周稳定期"的依赖正式化，技术层面 L2 POC 完成。

---

## § 10 Phase 3 Module 2 — LearningAgent (2026-04-19)

### 迁移概况
- **源文件**: `chaos/code/agents/learning_agent.py` (465 行)
- **LLM 调用点**: 仅 `_generate_recommendations()` — 迁移范围最小的模块
- **预计 ReAct cycles**: 1-2（实测平均 1 cycle）

### 架构决策
- 只有 `generate_recommendations()` 走 Strands ReAct
- 其余 4 方法（analyze / iterate_hypotheses / update_graph / generate_report）delegate 给 Direct
- Neptune helper 抽取到 `chaos/code/runner/neptune_helpers.py`（同时修复 hypothesis_tools.py 对 Direct 私有方法的依赖）
...
- 10 场景 × 4 分桶（核心服务 3 / 特殊后端 3 / 错误输入 2 / 边界 2）
- 行为约束验证：category 分布 / 推荐数量范围 / forbidden term

### Prompt Caching
- `CacheConfig(strategy="auto")`（不用 deprecated `cache_prompt`）
- System prompt: 1200+ tokens（含 fault type reference table）
- `assert_cacheable(min_tokens=1024)` 强制检查

### 集成测试
- `main.py learn --all` 端到端通过
- 86 条实验，89.5% 通过率，5 条建议
- `iterate_hypotheses()` 正确调用 `make_hypothesis_engine()`

### Commits
- `abbc801` — Week 1: LearningBase + DirectBedrockLearning + Golden Set
- `1d7186a` — Week 2: Strands impl + Golden CI 10/10
- Week 3: Wire factory + 集成 + 文档 + ADR
