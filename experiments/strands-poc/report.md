...
| strands | 19/20 = *95%*  | 95%  | 100% | 10152 ms | 17777 ms | — |
...
4. **q014 P0 过滤丢失**：Strands ReAct agent 把 "所有 P0 故障及其根因" 生成成 `MATCH (inc:Incident) RETURN ...`，跳过了 `severity = 'P0'` 过滤。不影响 19/20 达标，但是 Phase 2 稳定期要观察。
...
- [ ] `scripts/check_migration_deadlines.py` 接 GitHub Actions（当前未接）
- [ ] `.github/workflows/check-frozen.yml` 冻结文件 PR 检查
- [ ] q014 的 prompt 调优或补 few-shot，消除 ReAct 下 P0 过滤丢失
