...
# ── 清理旧产物（保留版控里的非产物文件）────────────────────────────────────
#
# 保留清单必须与「这个目录里哪些文件是源码/文档而非产物」保持一致。
# 2026-09-22：新增 GENERATED.md 时漏了这里，下一次构建会把它删掉 ——
# 而 tests/test_90_03 断言它存在，届时门禁变红；更糟的是警示文件一消失，
# 这份产物又能继续误导人对线上行为下结论（它已经误导过两轮审查）。
# 在此目录新增任何非产物文件时，**同时**把它加进下面的排除清单。
