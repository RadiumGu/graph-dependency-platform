...
┌─────────────────────┐   写入(mergeV/mergeE)   ┌──────────────────────┐   读取   ┌──────────────────────────┐
│   infra/ (CDK+ETL)  │ ─────────────────────→  │                      │ ───────→ │          rca/            │
│  4 个 Lambda 管道   │                          │   Amazon Neptune     │          │  多层 RCA 引擎            │
│  • DeepFlow  5min   │                          │   (openCypher)       │ ←─────── │  DeepFlow + CloudTrail   │
│  • AWS APIs  15min  │                          │                      │   写回   │  6 个 Layer2 探针        │
│  • EventBridge 实时 │                          │   23 种节点类型       │          │  Graph RAG 报告 → Slack  │
│  • CloudFormation 日│                          │   19 种边类型         │   读取   ┌──────────────────────────┐
└─────────────────────┘                          │   171+ 节点           │ ───────→ │         chaos/           │
                                                 │                      │          │  AI 假设生成（图谱驱动）  │
                                                 │   Q1–Q18 查询库      │ ←─────── │  61 种故障 × 双后端      │
                                                 │                      │   写回   │  闭环学习 Agent           │
                                                 │                      │          └──────────────────────────┘
                                                 │                      │   读取   ┌──────────────────────────┐
                                                 │                      │ ───────→ │    dr-plan-generator/    │
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                                   profiles/ + shared/   统一环境配置 & 服务注册（所有模块共享）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
<table style="border-collapse:collapse; width:100%; margin:1.5em 0; font-size:0.9em;">
  ...
  <tr>
    <td style="font-weight:bold; padding:10px 12px; border:1px solid #ddd; width:25%;">持续感知：实时构建知识图谱</td>
    <td style="font-weight:bold; padding:10px 12px; border:1px solid #ddd; width:25%;">事中：快速定位根因</td>
    <td style="font-weight:bold; padding:10px 12px; border:1px solid #ddd; width:25%;">事后：主动验证韧性</td>
    <td style="font-weight:bold; padding:10px 12px; border:1px solid #ddd; width:25%;">应急：有序执行容灾</td>
  </tr>
  <tr>
  ...
  <tr>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 实时依赖图谱</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 自动根因定位</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 主动故障注入</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 有序切换计划</td>
  </tr>
  <tr>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 爆炸半径分析</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 智能告警聚合</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ RCA 准确性验证</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 三层验证引擎</td>
  </tr>
  <tr>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 变更影响评估</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 分钟级 MTTR</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ DR 方案持续验证</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 策略驱动定制</td>
  </tr>
  <tr>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 依赖漂移检测</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ Graph RAG 报告</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 弱依赖自动发现</td>
    <td style="padding:6px 12px; border:1px solid #ddd;">▶ 关键路径 RTO 优化</td>
  </tr>
</table>
...
5. **dr-plan-generator/** 查询 Neptune 图谱，自动生成有序、可执行的 DR 切换计划，支持三层验证（dry-run → 逐步 → 全量演练）、策略驱动定制、Phase 2.5 就绪关卡，附带回滚指令和 RTO/RPO 估算
6. **profiles/** + **shared/** 提供集中式环境配置 — 所有模块从单一 YAML profile 加载服务映射、K8s 命名空间和资源标识符
...
│
├── profiles/       # ★ 环境配置 Profile（多应用支持）
│   ├── profile_loader.py   # EnvironmentProfile 加载器
│   └── petsite.yaml        # PetSite 应用 Profile
│
├── shared/         # ★ 跨模块共享组件
│   └── service_registry.py # 集中式服务名映射（Neptune ↔ K8s ↔ DeepFlow）
│
├── tests/          # 跨模块集成与回归测试
│
└── demo/           # Streamlit 演示看板
...
- **每步都有回滚**：不生成无回滚命令的步骤
- **环境 Profile**：所有服务映射从 `profiles/petsite.yaml` 加载

### 三层验证系统

| 层级 | 风险 | 功能 |
|------|------|------|
| L1 Dry-Run | 零 | 变量、资源、IAM、状态、网络、新鲜度、上下文 |
| L2 逐步验证 | 低 | 逐步执行 → 验证 → 回滚（3 种策略）|
| L3 全量演练 | 中 | 端到端演练 + Phase 2.5 硬阻断 + 自动回滚 |

### 三层策略系统

- **Layer 1**: `service_types.yaml` — 资源类型定义
- **Layer 2**: `plan_policy.yaml` / Markdown 策略 — 持久化定制 + NL 规则（LLM 解析）
- **Layer 3**: CLI `--set` 覆盖 — 最高优先级

### Phase 2.5：目标就绪关卡

5 项硬阻断检查：Tier0 副本就绪、ALB/NLB 目标健康、数据连通性、合成 E2E 测试、资源容量。任一失败阻止 Phase 3 执行。
...
## 5. profiles/ + shared/ — 环境配置

**集中式、Profile 驱动的配置，支持多应用切换。**

所有模块从 `profiles/petsite.yaml` 加载服务名映射、K8s 命名空间和资源标识符。切换 Profile 即可适配不同应用。

📖 **详情见**: `profiles/petsite.yaml` 和 `shared/service_registry.py`
