...
> 📄 **原路径**：`todo/project-intro-outline_20260831-0750.md`　**成文于**：2026-08-31 07:50

> ⚠️ **这份文档描述的是当时的状态，其中部分内容已被后续改动推翻。**
> 最明显的一类：文中若提到 `direct` 引擎或 direct/strands 双轨，那是迁移期的设定 ——
> **2026-09-20 起七个模块的 direct 实现已全部删除，factory 也去掉了回退分支，只剩 strands**。
> 文中指向 `*_direct.py` 的路径现已不存在。
...
| 其中**合法**（落在 7 种依赖边上） | **119** 条 | 94 | — |
| 其中**越界**（写在非依赖边上） | **0** 条（已清理，原 5 条） | 0（当时不存在此类边） | 见第 8 节 |
| **已被实验确证的依赖边** | **13 条 `confirmed`** | **0** | **从 0 到非 0** |
| 无法确证（`inconclusive`） | **14 条** | 0 | 新增 |
| 强度已分级（`verify_dependency_class`） | **6 条** | 0 | 新增 |
...
2. **验证覆盖率从 0 变成了非 0，但仍然很低——而这正是要讲的点。**
   119 条合法依赖边里 13 条确证、14 条 inconclusive，其余 92 条未尝试。强度维度只覆盖 6 条。
   （数字为 2026-09-06 实测；属性名是 `verify_status` 不是 `verify_verdict`。）
...
> 它解决的问题是：图里混着**被观测的系统**（petsite 业务）、**平台自身**（Neptune、6 个 ETL）、
...
- **验证覆盖率：119 条合法依赖边里 13 条确证、14 条 inconclusive、其余 92 条未尝试；强度维度只覆盖 6 条**（2026-09-06 实测）
