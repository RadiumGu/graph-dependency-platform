...
七条 ETL 的数据源天差地别 —— AWS Describe API、CloudWatch 日志、ClickHouse 里的
eBPF flow log、CloudFormation 模板 —— 但**骨架是同一个**。先拿到这个骨架，
后面两条具体链路才有参照。

```mermaid
flowchart LR
    SRC["数据源<br/>Describe API · 日志 · eBPF · 模板"]
    COL["① 采集<br/>子采集独立返回<br/>状态与数据分开"]
    GATE["② 门禁<br/>类型与 source<br/>必须已在契约声明"]
    UP["③ upsert<br/>fold + coalesce<br/>write-once 属性只首次写"]
    NEP[("Neptune")]
    DEC["④ 失活对账<br/>超 TTL → active=false<br/>不删"]
    CON["契约 graph_contract.yaml<br/>构建期冻结成常量"]
...
**① 采集** —— 每个子采集独立返回状态和数据，**不能用空列表表示失败**。
`[]` 是「确实没有」，失败是「我不知道有没有」，两者含义相反。混用会让权限丢失
表现成「图里资源消失了」而无人报警。

**② 门禁** —— 节点/边类型和 `source` 都必须在契约里声明过才能写。契约是构建期
冻结的常量，运行时只读。（各 ETL 实际的门禁覆盖面见第 12 节，那是另一回事。）

**③ upsert** —— 统一用 `fold().coalesce(unfold(), addV/addE(...))`。两个细节全仓库
一致：顶点属性必须带 `single`（否则默认 SET 基数，是追加不是覆盖）；
`source` / `first_seen` / `dependency_kind` 是写一次属性，记录「谁首先发现了这条
依赖」，不能被后写的源覆盖。

**④ 失活** —— 不删，置 `active=false`。阈值取「若干轮」而不是「一轮」，
容忍单轮漏采。硬删默认关闭。

### 看任何一条 ETL，先问三个问题

这三个问题决定了一条 ETL 长什么样，比记住每条的细节有用。

#### 问题一：身份键是现成的吗？

现成（ARN、资源 ID）→ 采到就能写。不现成（IP、主机名）→ 中间要加一步翻译，
而翻译引入一个新的失败模式：**采集成功但翻译失败，整轮无产出且现象像「没有流量」**。
`etl_deepflow` 要去问 EKS 才能把 IP 变成服务名，属于后者。

#### 问题二：这个源能给节点、给边，还是都给？

这是架构性约束，不是实现选择。**控制面 API 只能给节点** —— 它告诉你有哪些 agent，
不告诉你谁调了谁；调用关系只存在于运行时数据里（span、flow log）。
所以 `etl_agentcore` 必须开两条链路，而且两条的失败表现不同：控制面断了是
**节点缺失**，span 断了是**边缺失**（图上表现为「有 agent 但没有依赖」）。
混在一起实现会互相掩盖。

#### 问题三：这条依赖的 `dependency_kind` 该是什么？

```
static     配置/模板声明了，但不代表当前有流量
dynamic    持续观测到流量
inference  LLM 在运行时按 query 决定的调用
```

这个判断直接决定失活逻辑会不会误伤。判据是**调用形态而不是数据来源**：
`petsite → petsearch` 300 秒 25,042 次，「30 分钟没调用」确实说明变了；
而一天被调 35 次的 agent 工具，「6 小时没调用」什么也不说明 —— 把后者标成
`dynamic`，失活对账就会把客观存在的依赖判死。

### 留给听众的一句话

**每一处「简单写法」都在某次实测里给出过一个错误陈述，而不是缺失陈述。**
裸写 `source` 抹掉发现史、只 upsert 不失效让下线服务永久活跃、按 semconv 规范
假设属性名导致查询恒空 —— **错误陈述比没有陈述更难发现，因为它看起来是正常
工作的**。这正好回到项目主线：这张图是真的吗。
