...
> 📄 **原路径**：`todo/cloudwatch-cost-findings_20260829-0910.md`　**成文于**：2026-08-29 09:10
...
Container Insights 从修通那一刻起才真正开始计费。此前的 8 个 Pod 属纯消耗（详见 `containerinsights-iam-zero-writes.md`）。

四个 `/aws/containerinsights/PetSite/*` 日志组 retention 均为 **7 天**。

## 发现三：修通 Container Insights 的连带成本影响是双向的

正向（省）：观测自噪声引起的 DeepFlow 采集量下降 80.4%，ClickHouse 侧存储与查询成本随之下降（详见 `deepflow-noise-reduction.md`）。
