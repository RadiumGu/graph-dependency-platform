...
> 📄 **原路径**：`todo/neptune-selection-assessment_20260905-0650.md`　**成文于**：2026-09-05 06:50
...
1. **要撤销的论证**：不要再用「依赖分析需要多跳遍历，所以必须图数据库」为选型辩护。实测是有界 ≤10 跳、稀疏（max out-degree 14）、最深查询 234 条路径、图算法全在 Python 侧——这条理由经不起本文第二节的实测。对外介绍材料（`docs/design/project-intro-outline.md` §2「为什么用图数据库」）如果用了这条论证，应改为异构 schema + agent 可调用 + openCypher 方言风险三条。
