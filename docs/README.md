...
| [`large-scale-graph-refresh-architecture.md`](design/large-scale-graph-refresh-architecture.md) | 几十万节点／上百万边下「变更后依赖如何刷新」：Neptune 硬约束、边的生命周期（双时态+确认次数+分级 TTL）、派生结论三分类。含被否决方案与三条自我更正 |
...
| [`dr-korea-handover.md`](runbooks/dr-korea-handover.md) | **韩国守夜灯灾备站点交接文档 —— 接手这套东西先读这份**。先写「现在能做什么/不能做什么」再写怎么来的。最重要的一节是**四种「全绿但不能服务」的形态与唯一能区分的判据**（验收判据只有一条：逐个业务页面打一遍看 `<title>`）。另含接管/缩容的顺序约束及其因果（PDB 算术、零节点时集群拒绝创建任何 Service）、数据库切换的完整命令与三个容易弄错的 CLI 细节、以及未解决事项与关停命令 |
| [`dr-korea-switchover.md`](runbooks/dr-korea-switchover.md) | 东京→韩国切换操作手册。**第四节是最危险的一点：失效形态不是崩溃**；第五节是到「真能接管」还差什么的实测清点（非估算） |
