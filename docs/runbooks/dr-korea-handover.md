...
| **指标全绿**(最危险) | 数据库只读 12 分钟,payforadoption **168 次写入全部 500**,而 ALB 的 `HTTPCode_ELB_5XX_Count` / `Target_5XX` / `UnHealthyHostCount` **全部无数据**,请求量正常 | **直接看写入方(payforadoption)的日志** |

> ### ⚠️ 第五种为什么最危险
>
> 前四种至少有一个通道能看见。第五种**连告警通道都是绿的** ——
> 因为 **petsite 把后端 500 转成了自己的 200 错误页**(`txStatus=failure`)。
> ALB 只看到 200,所以 5xx 告警永远不响。
>
> 反讽的是:第三节那个健康检查修复(0/2 → 2/2)**正是用 ALB 指标核实的**,
> 在那里这个通道完全可信。**指标通道的可信度是按故障形态分的,不是按通道分的。**
>
> 两条配套陷阱:
>
> - **petsite 容器日志里也没有异常** —— 异常走 X-Ray,采样固定 5% + 蓄水池 1/秒,
>   未采样的请求连子段都不创建
> - **「X-Ray 上这条边没有流量」≠「没有调用」** —— 同窗口内 168 次真实调用,
>   全部失败且大多未被采样。**遥测缺失 ≠ 事件未发生。**
