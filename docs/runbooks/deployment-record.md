...
**日期**:2026-09-26 03:55–04:27 UTC ｜ 起因是用户要求「任何公网 IP 都不能直接暴露」

#### ① 公网暴露面审计(先说结论)

```
韩国 ap-northeast-2    公网 IP 的 EC2  0 台      internet-facing LB  0 个     ✅
东京 ap-northeast-1    公网 IP 的 EC2  1 台      internet-facing LB  4 个
    其中 3 个 ALB + 那台 EC2 属于别的项目（devops-agent-cn-bridge / openclaw / Transl），
    不是我建的 —— 只报不动
```
...
| 资源 | 状态 |
|---|---|
| 东京公网 ALB `Servic-PetSi-by0kpyBtxswj` | 有 Cognito,**但有 `X-Demo-Bypass` 请求头旁路可完全跳过认证**(优先级 1/2/3,非我所建) |
| 东京 API 网关 `9dw5r2dqlb` | `authorizationType=NONE`、无 authorizer、无 API key、无资源策略 —— **完全开放的写接口** |
| 韩国 API 网关 `dcwctbprp5` | 有资源策略按 IP 收窄 ✅ |

#### ② 事故一:我用一次「验证探测」打停了东京的领养流程

**时间线**

```
~03:57  给 9dw5r2dqlb 加资源策略（按东京两个 NAT 的 EIP 收窄）+ 重新部署
~03:58  发探测请求验证是否已关闭 → **HTTP 200**（策略还没传播）
        请求体 {"pettype":"puppy","petid":"NONEXISTENT-verify-closed"}
        → Lambda 的 UpdateItem **upsert 出一条只有 3 个属性的行**
        （只有 pettype / petid / availability，缺 cuteness_rate / image / petcolor / price）
~04:00  策略生效，探测开始返回 403 —— **我以为这就是验证通过，转去看指标**
        **那条畸形行留在了生产表里，我没清理**
03:55→  search-service 扫表时对缺失字段 NullPointerException → /api/search 返回 500
        → petsite 取不到宠物 → traffic-generator 的 LoadPetData 失败
        → **领养完全停止**
~04:17  删掉畸形行
~04:19  NPE 归零、500 归零、领养恢复（两个 pod 各 4 次/3 分钟）
```

**影响面**:约 22 分钟内领养流程完全停止。API 网关请求量从每 5 分钟 24 次掉到 0。

##### 这是同一个错误的**第二次**,而且我 40 分钟前刚把教训写进 4.34

4.34 里我自己写的:

> **「演示一个漏洞存在」和「利用它」之间只差一次请求**,而我选择的是对生产发真请求。
> 更稳妥的做法是只读 `describe-rest-api` 的授权配置就下结论。

然后我在 40 分钟后**又发了一次**,这次还忘了清理。

> **把教训写下来不等于会照着做。** 第一次我至少立刻清理了;第二次因为
> 「探测返回 403 了」就认为验证完成,注意力转移,遗留物被留在生产表里。
> **真正的防线是「不对生产发写请求」,而不是「写完记得清理」。**

##### 我的第一次诊断也是错的 —— 又一次子串误配

看到 traffic-generator 报 500,我 grep payforadoption 日志里的 `403`,得到 **16 个命中**,
一度判断「我的策略打断了调用方」。实际那 16 个是**时间戳里的数字**
(`03:50:33.454033604Z` 含 `403`)。

用带边界的模式重查后是 **0 个真 403**。**「子串误配」这条我也写过,也又踩了一次。**

#### ③ 事故二:IP 白名单造成稳定 1/3 的静默拒绝

策略生效后指标稳定在:

```
Count 24    4XX 8      ← 每 24 次请求有 8 次被拒，稳定的 1/3
```

我放行了东京两个 NAT 的 EIP(`54.250.45.148`、`13.159.132.88`),
实测两个节点子网各走一个。但仍有 1/3 被拒 —— **存在第三条出口路径我没找到**。

**这个失效形态正是最坏的一种**:领养成功、页面正常、日志无错,
只有 1/3 的宠物状态不更新。**没有任何症状会让人去查。**

**已回滚**,4XX 回到 0、Count 稳定 24。

> **教训**:按出口 IP 做白名单,正确性取决于「我是否穷举了所有出口路径」——
> 而**穷举失败的表现是静默部分失败,不是报错**。
> 韩国那个网关能用这招是因为韩国只有一个 NAT 且我验证过 pod 子网都走它;
> 东京有两个 NAT 且拓扑更复杂,同一招就不可靠了。
> **「同一个手法在 A 处可靠」不代表在 B 处可靠 —— 可靠性来自那次验证，不是手法本身。**

#### ④ 所以东京那个网关**仍然是开放的**

我没有硬推第二次。按出口 IP 收窄这条路在东京已被证明不可靠,
正确的做法是**不依赖猜 IP**:

```
把 9dw5r2dqlb 改成 private REST API + execute-api 接口 VPC 端点
  - 资源策略用 aws:SourceVpc（确定的，不用穷举 IP）
  - 开启私有 DNS 后调用方 URL 不变 → 一行代码不用改
  - 代价:接口端点约 $7–15/月（两个 AZ）
```

这是个改动量更大、需要停一步确认的方案,已交给用户决定。

#### ⑤ `X-Demo-Bypass` 旁路我也没动
