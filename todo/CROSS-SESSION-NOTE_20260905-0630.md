...
- `test_s5_08`：`estimated_rpo_minutes == 0` 而 `total_affected=5`、
  ...
  **0 与「算不出来」同形**，建议让它在算不出时返回 None 或显式抛，
  ...
  `docs/lessons/injection-found-defects.md` #27/#30。）
...
## 2026-09-20 — 跨会话统一核对:线上是干净的,乱在**契约与实现脱节**

3-4 个会话并发改了本地代码和线上环境之后的一次全面核对。
结论与直觉相反:**线上没有乱,乱在单一事实来源没跟上实现。**

### 线上七项检查全部通过

    遗留 ChaosDenyProbe IAM 策略     无
    进行中的 FIS 实验                 无
    处于 ALARM 的告警                 0
    K8s Pod（petadoptions）           13 个全 Running
    混沌遗留对象（NetworkPolicy 等）   无
    临时/测试 Lambda                  无
    近 3 天被改的 Lambda              0 个

09-17 那两次 IAM deny 实验（我之后另一个会话跑的）都
`policy_removed=True`。**故障注入的自动恢复纪律是有效的** ——
多个会话各自跑实验,没有一次留下未恢复的故障。这一点值得记下来,
因为它说明「必须可自动恢复」这条硬约束真的在起作用,不是纸面要求。

### 真正的问题:三次 status 漂移,门禁一次都没报警

图谱里 `verify_status` 实际在用 7 个值,而契约
`profiles/graph_contract.yaml` 的 `statuses` 停在 4 个。

而 `tests/test_67:76` 那条断言**只校验契约文件里的列表**,
不校验图谱里实际写着什么。

**盲区的形状值得记住:门禁守着"契约自己说了什么",
没守着"实现是不是照契约做的"。契约与实现脱节时它永远是绿的。**

已补 `test_t67_01b`:从图谱取实际取值集合与契约求差集。
反向验证过它真能抓到漂移（模拟契约漏登记 `modeling_artifact`
时差集非空、门禁会红），不是只看它绿了就算完。

### 三个越界值的甄别 —— 两个是对的,一个是我的错

我一开始怀疑它们是野生数据,核实后要分开看:

· **`modeling_artifact`**（5 条）合法。语义是"源码证明这条依赖不可能
  发生",带 file:line 证据,写入受 `classify_modeling_artifacts.py`
  三条硬约束（声明式白名单 + evidence 必填 + searched 必填 + 绝不删边）。
  它与 `refuted` 同为"不成立",区别只在证据来源。**是证据陈述,属于这个轴。**

· **`bootstrap_only`**（2 条）合法。`edge_verification.py:437` 那份
  三轴定义里明确列着它。

· **`soft`**（1 条）**是我写错的**。它属于 `verify_dependency_class`
  （承重程度 hard/degraded/soft/unclassified）,不属于 `verify_status`。
  我 09-17 把承重程度写进了证据字段 —— 正是
  `mark_granularity_duplicates.py` 的注释警告过的那件事:
  「塞进去就是又一次把两套轴挤进一个字段」。

契约已补登记前两个,并写明**不要**加 `soft` 及其原因。

### 改这条边时被另一条门禁反驳了两次,两次它都对

第一次我改成 `verify_status='confirmed' + dependency_class='soft'`。
`tests/test_66::test_m02` 立刻红:

> 这些边有 confirmed/refuted 判定却没有退化幅度。
> 判定只能来自干预,而干预必然测到一个退化值。

它是对的。我这条边**干预成功**（源侧 379 次 403）但**业务无感**,
所以没有退化值 —— 那就不满足 confirmed。

最终落到 `verify_status='inconclusive' + verify_dependency_class='soft'`,
reason 里写清「不是『没测出来』,而是『测到切断生效但无业务退化』」。

顺带暴露一个**真实的词汇缺口**:「切断确实生效 + 业务确实无感」这个结论,
在现有 status 词汇里没有独立位置,只能表达成 inconclusive + soft。
这不算错,但读的人容易把 inconclusive 读成"实验失败了"。
要不要给它一个独立取值,是个需要想清楚再动的设计问题 ——
**不要因为一条边就扩词汇**,那正是这次漂移的来源。

### 可迁移的两条

1. **多会话并发下,先查线上再查本地。** 未恢复的故障注入是唯一
   不可逆的风险,而它恰好最容易查（IAM 内联策略 / FIS 状态 /
   Pod 健康 / 告警状态,四条命令）。本地的乱都是可修的。
