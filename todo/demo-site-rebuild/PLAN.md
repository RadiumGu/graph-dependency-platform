...
- [x] **T3b 图确实需要横向拖动 —— 但那句提示说了反话** ✅ 2026-09-08

  ### 需要拖动,而且这是刻意的取舍

  实测:内容 x 跨度 **1615px**,主容器约 900px。`fitReadable()` 先调
  `network.fit()`,若结果缩放低于 `MIN_SCALE = 0.75` 就**放弃装下全图**,
  改为保持可读比例并对准锚点。这里 fit 需要 0.56 倍 → 钳到 0.75 →
  **约 26% 的内容在视口外**。

  取舍本身是对的:fit() 为了装下全图会无限缩小,节点标签变成糊点,图就白画了。

  ### 但提示是错的,而且在需要时不出现

  原文:`if layered and widest > 12: st.caption("画布已自动缩放到能装下全图…")`

  两个问题:
  - **说反话**。`fitReadable` 明确地在这种情况下不装下全图。
    用户看到一张右侧被切掉的图 + 一句「已装下全图」,只会得出「这图是坏的」。
  - **触发条件是代理量**。`widest > 12` 数的是最宽一层的节点数;
    实测那个 20 节点视图最宽层只有 10 个 → 提示不出现,而 26% 内容在视口外。
    **需要拖动时提示反而不显示。**

  改用**真实 x 跨度**判断,并把取舍讲清楚。现在的提示:

  > 图的横向跨度约 1615px,比画布宽 —— **约 26% 的内容在视口外,按住空白处拖动
  > 即可看到。** 这是刻意的取舍:自动缩放到装下全图会让缩放降到 0.56 倍,
  > 节点标签变成糊点、图就白画了。所以缩放保持在可读下限 0.75 并对准锚点。

  门禁 `tests/test_63_graph_canvas_hint_is_truthful.py`(4 条,
...
- [x] **T6 两个 Explorer 都保留** ✅ 2026-09-08 —— 不是重复:pyvis 从根本上是
  **单向**的,选中节点拿不回 Python,所以「点节点→展开邻居/出侧栏详情」在它上面
  ...
  (Neo4j Bloom / AWS graph-explorer 也都同时提供)。删任一个都丢掉一种成熟模式。
- [x] **T7 Query_Catalog 独立保留** ✅ 2026-09-08 —— 那 4 个精选查询是
  **免 AI、点一下就有结果**的硬证据,而且它正是「AI 是不是在编」的对照手段:
  同一个问题用固定 Cypher 跑一遍。与自然语言问答是互补,不是冗余。
- [x] **T8 首页摆出论证线 + 导航按论证线分组** ✅ 2026-09-08

  **真正的问题从来不是页数,是扁平清单里读不出论证。** 侧栏原来是十条扁平清单,
  十个页面各自都能用,但连起来读不出一条线 —— 访客看到的是「这个项目有十个功能」。

  导航改成四段(`NAV_GROUPS`),顺序不能换:
  ...
  首页新增「怎么读这个站」一节,每段说清**为什么需要它**,并直接链到对应页面。
  顺序是:主张 → 计分板(第一手证据)→ 路线图。

  `NAV` 扁平清单改为从 `NAV_GROUPS` 推导 —— 两份手写清单必然漂移。

  门禁 `tests/test_61_site_structure_argument_line.py`(5 条,
  **m01/m02/m03/m05 已反向验证**),其中 **m02 挡「页面文件不在导航里」** ——
  本项目用自定义导航,漏掉一页不报任何错,它就是静静地消失。
...
- [x] **T11b 虚构 AZ 名:按危害分三类处理,不一律替换** ✅ 2026-09-08

  全仓 195 处、20 个文件。但**不能一律搜索替换**:

  | 类别 | 处理 | 理由 |
  |---|---|---|
  | 文档(README×2 / AGENT.md / SKILL.md / prd / tdd / references) | **改成真实名** | 里面是**可复制粘贴的命令行**,照着跑得到「受影响服务 0」的空计划;读 AGENT.md 的 agent 会照着传参,然后拿空计划继续推理 |
  | `examples/*.json|md`(约 109 处) | **不动,但标明是合成的** | 它们是**历史产物**。替换 AZ 名会造出「声称来自 `ap-northeast-1a`、实际描述另一套拓扑」的**假产物** —— 比留着假名更糟 |
  | `tests/` 与 `fixtures/` | 不动 | 合成数据本就该用不会与现实混淆的名字;真实名的覆盖由 `test_60` 负责(直接打活图谱) |

  顺带更正 README 一处误导:原文写「**基于 PetSite 拓扑**的预生成示例」,
  而它们其实来自 `generate_examples.py` 里一份**硬编码的合成节点表**。
  现在标明是合成的,并给出真实图谱的实测数字作对照
  (`1a` → 6 个服务 / `1c` → 7 个含 1 个全停 / `1d` → 0)。

  `generate_examples.py` 开头也补了标注,并写明**不要**用搜索替换「修」AZ 名
  以及为什么 —— 它是那批假产物的源头,下一个改它的人该一眼看到。

  门禁 `tests/test_62_no_fictional_az_in_docs.py`(4 条,m01/m02/m03 已反向验证)。

  ⚠️ **判据第一版又错了(本会话第九次同族)**:写成「文件里出现 `apne1-azN` 就违规」,
  结果匹配到我自己写的那两行警告 —— 那两行的内容恰恰是「这些名字是假的」。
  改成对准**语法位置**:只看 `--source` / `--target` / `--failure` 后面的参数值,
  即「照着能跑」的那种出现。
