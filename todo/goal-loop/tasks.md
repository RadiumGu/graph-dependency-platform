...
- 2026-08-28T22:15Z cycle-22 收口: 最后做了一件只读但能定案的事 —— 软删除观测。
  4.6 小时、ETL 调用 73 次:18 条 Calls 边稳定在 1 active / 17 inactive,
  活跃边 last_seen 0 分钟前(每轮刷新),最"新"的失效边 59.6 天前 ——
  相对 1800s 阈值有 **2860 倍余量,零误判零翻转**。
  **硬删除建议不启用**,且 T-030 之后出现了一条新的反对理由:变更日志记录
  「依赖何时停止被观测」,而失效边本身承载「它曾经存在」;硬删除删掉后半句,
  变更事件将指向一条不存在的边。**T-030 让保留失效边从「无害」变成「有用」。**
  循环已达成全部 5 条 DoD(cycle-8)并按用户指示清完所有我能独立完成的卡片。
