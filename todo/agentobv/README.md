...
**调研日期**：2026-09-04
**输入**：`../research-agent-dependency-graph.md`（图谱扩展到 GenAI Agent 的可行性调研）
...
| 文件 | 内容 |
|---|---|
| `01-agent系统接入方案.md` | 问题一：怎么在 PetSite 里加 agent 系统 |
| `02-agent可观测性方案.md` | 问题二：怎么观测它，怎么接进现有依赖图谱 |
| `03-可复用资产与证据清单.md` | 证据底座、可复用代码、**以及查不到什么**（第 4.1 节已撤销重写） |
| `04-FIS模板库与agent注入_20260904-0830.md` | **推翻了 `03` 一个结论**：agent 注入该用 FIS 编排；含四个陷阱（三个会产假 PASS） |
| `05-etl_xray影响面量化.md` | 执行顺序第 1 步的**产出**：影响面极窄、TS 当前未开、基线已记录、残余一个未知含可逆实测方案 |
