...
| 5 | `dependency_kind` 现状 | ❌ 文档写错 | 文档说现有取值是 `dynamic`/`static`，实际是 **`static`/`observed`**（`dynamic` 出现 0 次） |
...
>
> **本节的一处事实性错误**：说「现有两个取值 `dynamic` / `static`」。
> 实测契约里 `dynamic` 出现 **0 次**，实际的两个取值是 **`static` / `observed`**。
> 建立在错误前提上的「需要第三个取值 `inference`」这个论证因此需要重新评估——
> 而且如上所述，它想解决的问题已由「采集窗口 = TTL」这条约束解决了。`inference` 未落地，**这是对的**。
