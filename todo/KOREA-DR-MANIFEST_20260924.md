...
### ⚠️ 更正:「拆 NAT 省 41/月」这条对完整站点**不成立**

2026-09-24 建完 EKS 后发现的:节点(将来拉起时)靠 NAT 访问 ECR / S3。
**拆了 NAT,EKS 节点就拉不了镜像。**

若要拆 NAT,必须先补这些 VPC 端点:`ecr.api`、`ecr.dkr`、`s3`(网关型,免费)、
`sts`、`elasticloadbalancing`、`autoscaling` —— 约 5 个接口端点
(各 ~7-8/月)**算下来比 NAT 还贵**。

**结论:NAT 保留,常态成本按 ~211/月 计。**
「拆 NAT」只对「站点里只有 Temporal」那个中间阶段成立,不适用于完整站点。
